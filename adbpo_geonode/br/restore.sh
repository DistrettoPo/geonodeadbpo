...
if md5sum -c /$BKP_FOLDER_NAME/$NEW_UUID/$BKP_FILE_NAME.md5; then
    ...
    if md5sum -c /$BKP_FOLDER_NAME/$BKP_FILE_NAME.md5; then
        # The MD5 sum matched
        ./manage.sh restore -c adbpo_geonode/br/settings_docker.ini -n -f --backup-file /$BKP_FOLDER_NAME/$BKP_FILE_NAME.zip --recovery-file /$BKP_FOLDER_NAME/$NEW_UUID/$RECOVERY_FILE_NAME.zip
